...
func Validate(records SenMLMessage) error {
        var bn string = ""
        ...
        for i, record := range records {
                ...
                if strings.HasPrefix(record.Bn, "urn:dev:mac:") {
                        fmt.Printf("bn(MAC): %s, n: %s\n", record.Bn, record.N)
                        parts := strings.Split(record.Bn, ":")
                        //for i, part := range parts {
                        fmt.Printf("\tMAC: %s\n", parts[3])
                        //}
                } else if strings.HasPrefix(record.Bn, "urn:dev:sn:") {

                } else if strings.HasPrefix(record.Bn, "urn:sys:name:") {
